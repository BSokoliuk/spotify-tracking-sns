...
function Charts() {
  ...
  const getPopularSongs = async (i) => {
    ...
    const response = await postData("scrobbles/top-songs-all", {
      start: interval,
      end: new Date(),
      n: 10,
    });
  ...
  const getPopularAlbums = async (i) => {
    setPopularAlbums([]);
    const interval = getInterval(i);
    const response = await postData("scrobbles/top-albums-all", {
      start: interval,
      end: new Date(),
      n: 10,
    });
  ...
  const getPopularArtists = async (i) => {
    setPopularArtists([]);
    const interval = getInterval(i);
    const response = await postData("scrobbles/top-artists-all", {
      start: interval,
      end: new Date(),
      n: 10,
    });
            ...
            {likedSongs().length > 0 && (
              <>
                <div class="my-5 flex flex-col md:flex-row xl:justify-center xl:items-center xl:mx-auto">
                  ...
                  <img
                    onClick={(event) =>
                      handleClick(event, "song", likedSongs()[0].song.title)
                    }
                    src={`data:image/png;base64,${
                      likedSongs()[0].song.album.cover
                    }`}
                    alt="cover"
                    class="mx-auto md:mr-0 md:ml-2 w-32 h-32 rounded-md cursor-pointer"
                  />
                  <div class="ml-3 flex flex-col">
                    <p
                      ...
                      class="mr-2 truncate w-72 lg:w-56 xl:w-60 font-bold cursor-pointer"
                    >
                      {likedSongs()[0].song.title}
                    </p>
                    <p
                      onClick={(event) =>
                        handleClick(
                          event,
                          "artist",
                          likedSongs()[0].song.album.artist.name
                        )
                      }
                      class="mr-2 truncate w-72 lg:w-56 xl:w-60 cursor-pointer"
                    >
                      {likedSongs()[0].song.album.artist.name}
                    </p>
                  </div>
                ...
                <div class="grid grid-cols-1 xl:grid-cols-3 mt-5">
                  {likedSongs()
                    ...
                    .map((song, index) => (
                      <ChartsRow
                        photo={song.song.album.cover}
                        textMain={song.song.title}
                        textSecondary={song.song.album.artist.name}
                        type={"song"}
                        typeSecondary={"artist"}
                        index={index + 1}
                      />
                    ))}
                </div>
